...
# named after the current date. ex: 2016-04-25.tar.gz
# sudo  crontab -e
# 0 17 * * * /bin/bash /home/ubuntu/backup/mongodb-backup.sh >/dev/null 2>&1
#
...
BACKUP_PATH="$HOME/backup/files" # do not include trailing slash
FILE_NAME="DATE" #defaults to [currentdate].tar.gz ex: 2016-04-25.tar.gz
...
if [ -d "$BACKUP_PATH" ]; then
	...
	if [ "$USERNAME" != "" -a "$PASSWORD" != "" ]; then
		$MONGO_DUMP_BIN_PATH --host $HOST:$PORT --ssl --sslAllowInvalidCertificates --authenticationDatabase admin -u $USERNAME -p $PASSWORD --out $TMP_BACKUP_DIR >> /dev/null
	else
		$MONGO_DUMP_BIN_PATH --host $HOST:$PORT --ssl --sslAllowInvalidCertificates --authenticationDatabase admin --out $TMP_BACKUP_DIR >> /dev/null
	fi
